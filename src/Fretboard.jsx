...
import {
    createNoteClickHandler,
    createNoteContextMenuHandler,
    createDeleteNoteHandler,
    createFinishEditingHandler,
} from "./handlers/noteHandlers";
import {
    createSvgClickHandler,
    createSvgContextMenuHandler,
    createSvgMouseMoveHandler,
    createSvgMouseDownHandler,
    createSvgWheelHandler,
    createEditableKeyDownHandler,
    createEditableClickHandler,
} from "./handlers/svgHandlers";
...
function Fretboard() {
    ...
    // 计算函数
    const computeNoteIndexMemo = useCallback(
        (fret, string) => computeNoteIndex(fret, string),
        [],
    );
    const computeNoteNameMemo = useCallback(
        (fret, string) => computeNoteName(fret, string, enharmonic),
        [enharmonic],
    );
    ...
    const markers = useMemo(
        () => generateMarkers(startFret, endFret),
        [startFret, endFret],
    );
    const fretPath = useMemo(
        () => generateFretPath(startFret, endFret),
        [startFret, endFret],
    );
    const connections = useMemo(
        () => (data && data.connections ? data.connections : {}),
        [data],
    );

    // 获取note位置
    const getNotePositionMemo = useCallback(
        (noteId) => getNotePosition(noteId, notes),
        [notes],
    );
    ...
    useEffect(() => {
        if (toolbarDropdown && toolbarRef.current) {
            setTimeout(() => {
                ...
                const button =
                    toolbarRef.current?.querySelector(buttonSelector);
                if (button) {
    ...
    // 连线相关处理器
    const openConnectionToolbarMemo = useCallback(
        (connectionId) => {
            openConnectionToolbar(
                connectionId,
                connections,
    ...
    const handleConnectionContextMenuMemo = useCallback(
        (event, connectionId) => {
            handleConnectionContextMenu(
                event,
                connectionId,
    ...
    const handleConnectionClickMemo = useCallback(
        (event, connectionId) => {
            handleConnectionClick(
                event,
                connectionId,
    ...
    // 工具函数
    const selectColorMemo = useCallback(
        (level, color, customColor = null) => {
            selectColor(
                level,
                color,
                selectedColorLevel,
    ...
    const doubleClickColorMemo = useCallback(
        (level, color) => {
            const baseColor =
                level === 1 ? getLevel1FillColor(color) : getLevel2Color(color);
            ...
            for (const [key, value] of Object.entries(newData)) {
                ...
                if (
                    ...
                    newData[key] = {
                        ...value,
                        visibility:
                            visibility === "hidden" ? "transparent" : "hidden",
                    };
                }
            }
            return newData;
    ...
    const replaceAllTintNotesMemo = useCallback(
        (targetColorName) => {
            replaceAllTintNotes(
                targetColorName,
                data,
    ...
    const setFretWindowMemo = useCallback(
        (fretWindow) => {
            setFretWindow(
                fretWindow,
                startFret,
    ...
    const saveFretboardStateMemo = useCallback(
        (forceNew = false) => {
            const now = Date.now();
            ...
            if (selectedHistoryState && !forceNew) {
                ...
                const index = updatedStates.findIndex(
                    (item) => item.id === selectedHistoryState.id,
                );
                if (index !== -1) {
            ...
            if (isUpdate) {
                // 更新现有状态
                storageService.updateState(stateSnapshot.id, {
                ...
                // 创建新状态
                storageService.createState(stateSnapshot);
            }
    ...
    // 恢复指板状态
    const restoreFretboardStateMemo = useCallback(
        (stateSnapshot) => {
    ...
    // 包装目录操作函数，直接使用本地状态
    const wrappedCreateDirectory = useCallback(
        (baseName = "new") => {
            const result = createDirectory(baseName);

            // 同步保存到 localStorage
            if (result && result.id) {
                storageService.createDirectory(result);
            }
    ...
    const wrappedRenameDirectory = useCallback(
        (dirId, newName) => {
            const result = renameDirectory(dirId, newName);

            if (result.success) {
                // 同步更新到 localStorage
                storageService.updateDirectory(dirId, {
    ...
    const wrappedDeleteDirectory = useCallback(
        (dirId) => {
            const result = deleteDirectory(dirId);

            if (result.success) {
                // 先更新该目录下的所有状态到default目录
                const statesToUpdate = historyStates.filter(
                    (state) => state.directoryId === dirId,
                );
                statesToUpdate.forEach((state) =>
                    storageService.updateState(state.id, {
                        directoryId: "default",
                    }),
                );

                // 然后删除目录
                storageService.deleteDirectory(dirId);
            }

            return result;
        },
        [deleteDirectory, historyStates],
    );
    ...
    handlerParamsRef.current = {
        selected,
        deleteNote,
        selectColor: selectColorMemo,
        cycleLevel1Color: cycleLevel1ColorMemo,
        cycleLevel1ColorReverse: cycleLevel1ColorReverseMemo,
        cycleLevel2Color: cycleLevel2ColorMemo,
        cycleLevel2ColorReverse: cycleLevel2ColorReverseMemo,
        undo,
        redo,
        hoveredNoteId,
        hoveredConnectionId,
        ...
        setPreviewHoverNote,
        setUseColor2Level,
        saveFretboardState: saveFretboardStateMemo,
        toggleVisibility: toggleVisibilityMemo,
        reset: resetMemo,
        saveSVG: saveSVGMemo,
        selectedColorLevel,
    };
    ...
    // 生成字符串路径
    const generateStringPathMemo = useCallback(
        (stringIndex) => generateStringPath(stringIndex, fretboardWidth),
        [fretboardWidth],
    );
    ...
    useEffect(() => {
        ...
        // 从 localStorage 加载
        const { directories: loadedDirs, states: loadedStates } =
        ...
        <>
            <div className="title-header">
                <div>
                    ...
                    {selectedHistoryState && (
                        <>
                            <div
                                className="selected-state-name"
                                title="当前选中的历史状态,保存将更新此状态"
                                style={{
            ...
            <FretboardGallery
                ...
                onDelete={(stateSnapshot) => {
                    // 使用增量更新删除状态
                    storageService.deleteState(stateSnapshot.id);
                    const updatedStates = historyStates.filter(
                        (item) => item.id !== stateSnapshot.id,
                    );
                    setHistoryStates(updatedStates);
                    if (
                        selectedHistoryState &&
                        selectedHistoryState.id === stateSnapshot.id
                    ) {
                ...
                onClearAll={() => {
                    ...
                    historyStates.forEach((state) =>
                        storageService.deleteState(state.id),
                    );
                    setHistoryStates([]);
                    setSelectedHistoryState(null);
                    ...
                    requestAnimationFrame(() => {
                        setTimeout(() => {
                            ...
                            const existingIndex = updatedStates.findIndex(
                                (item) => item.id === importedState.id,
                            );

                            if (existingIndex !== -1) {
                                // 更新现有状态
                                storageService.updateState(importedState.id, {
                                ...
                                // 创建新状态
                                storageService.createState(importedState);
