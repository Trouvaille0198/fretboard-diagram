...
// 恢复指板状态
export function restoreFretboardState(
	stateSnapshot,
	{
		setData,
		setStartFret,
	...
	try {
		...
		const restoredData = savedState.data
...
export function validateDirectoryName(name, directories, currentDirId = null) {
	...
	const isDuplicate = directories.some(
		(dir) =>
			dir.id !== currentDirId &&
			dir.name.toLowerCase() === trimmedName.toLowerCase()
	);
...
export function exportAllData() {
	try {
		...
		const historyStates = JSON.parse(
			localStorage.getItem("fretboard-history") || "[]"
		);
		...
		const exportData = {
			...
			exportTime: Date.now(),
			directories:
				directories.length > 0 ? directories : initializeDirectories(),
			historyStates,
		};
		...
		const now = new Date();
		const filename = `fretboard-backup-${now.getFullYear()}${String(
			now.getMonth() + 1
...
export function mergeDirectories(localDirs, importDirs) {
	...
	for (const importDir of importDirs) {
		...
		const existingDir = localDirs.find(
...
export function importBatchData(jsonData) {
	try {
		...
		const localStates = JSON.parse(
			localStorage.getItem("fretboard-history") || "[]"
		);

		// 确保有默认目录
		const directories =
			localDirs.length > 0 ? localDirs : initializeDirectories();

		// 合并目录
		const { dirMapping, newDirs, mergeCount } = mergeDirectories(
			directories,
			jsonData.directories
		);
		const updatedDirs = [...directories, ...newDirs];

		// 解决状态冲突
		const resolvedStates = resolveStateConflicts(
			localStates,
			jsonData.historyStates,
			dirMapping
		);
