...
export function selectColor(
    ...
    // 如果有自定义颜色，存储为对象
    setSelectedColor(
        customColor ? { name: color, custom: customColor } : color,
    );
}
...
export function replaceAllTintNotes(
    ...
    for (const [noteId, noteData] of Object.entries(newData)) {
        ...
        if (updateColor || updateColor2) {
            ...
            // 更新DOM元素
            if (notesElementRef.current) {
                const noteElement = notesElementRef.current.querySelector(
                    `#${noteId}`,
                );
                if (noteElement) {
...
export function saveSVG(
        ...
        if (!showNotes) {
            ...
            connections.forEach((conn) => {
                ...
                if (path) {
                    ...
                    const transNoteIds = Array.from(transNotes).map(
                        (n) => n.id,
                    );
    ...
    noteElements.forEach((noteElement) => {
        ...
        const hasColor =
            noteData.color &&
            (noteData.visibility === "visible" ||
                noteData.visibility === "selected");
    ...
    if (!verticalCrop) {
        ...
        const calculateVerticalBounds = () => {
                ...
                if (markersGroup) {
                    ...
                    if (markers.length > 0) {
                        try {
                            // 尝试从原始DOM获取计算样式
                            const originalSvg =
                                document.getElementById("fretboard-svg");
                            if (originalSvg) {
                                const originalMarker =
                                    originalSvg.querySelector("text.marker");
                                if (originalMarker) {
                                    const computedStyle =
                                        window.getComputedStyle(originalMarker);
                    ...
                    try {
                        ...
                        if (originalMarker) {
                            const computedStyle =
                                window.getComputedStyle(originalMarker);
                            dominantBaseline =
                                (computedStyle.dominantBaseline || "").trim() ||
                                "auto";
                        }
                    } catch (e) {
                        // ignore
                    ...
                    const ascent = isMiddleBaseline
                        ? markerFontSize / 2
                        : markerFontSize * 0.8;
                    const descent = isMiddleBaseline
                        ? markerFontSize / 2
                        : markerFontSize * 0.2;
                    ...
                    markers.forEach((marker) => {
                        ...
                        if (!isNaN(currentY)) {
                            ...
                            if (currentY < CONSTS.offsetY) {
                                ...
                                currentY >
                                CONSTS.offsetY + CONSTS.fretHeight
                            ) {
                                // 底部marker - 使用文本的下边缘
                ...
                noteElements.forEach((noteElement) => {
                    ...
                    if (stringIndex === 0) {
                        ...
                        if (!isHidden) {
                            ...
                            let noteY = parseFloat(
                                noteElement.getAttribute("data-y"),
                            );
                            if (isNaN(noteY)) {
                                const transform =
                                    noteElement.getAttribute("transform");
                                if (transform) {
                                    const match = transform.match(
                                        /translate\([^,]+,\s*([^)]+)\)/,
                                    );
            ...
            if (bottomEdge === null) {
                bottomEdge =
                    CONSTS.offsetY +
                    CONSTS.stringSpacing * (CONSTS.numStrings - 1) +
                    PADDING;
            }
        ...
        if (
            ...
            // 根据string索引计算Y坐标范围
            const topStringY =
                CONSTS.offsetY + CONSTS.stringSpacing * minStringIndex;
            const bottomStringY =
            ...
            const calculateSingleEdge = (isTop) => {
                ...
                if (includeMarkers) {
                    ...
                    if (markersGroup) {
                        const markers =
                            markersGroup.querySelectorAll(".marker");
                        ...
                        try {
                            ...
                            if (originalMarker) {
                                ...
                                dominantBaseline =
                                    (
                                        computedStyle.dominantBaseline || ""
                                    ).trim() || "auto";
                            }
                        } catch (e) {
                            // ignore
    ...
    if (
        ...
        if (minFret === 0) {
            ...
            minX =
                CONSTS.offsetX +
                CONSTS.fretWidth * (minFret - (startFret || 0));
        }

        // 右边界：最大品丝的右边界（下一品丝的位置）
        maxX =
            CONSTS.offsetX +
            CONSTS.fretWidth * (maxFret + 1 - (startFret || 0));
    ...
    if (copyOnly) {
                        ...
                        .catch((err) => {
                            ...
                            if (setToastMessage) {
                                ...
                                if (
                                    ...
                                    setToastMessage(
                                        t?.actions?.toastCopyPermission ??
                                            "Copy failed: clipboard permission required.",
                                    );
                                } else {
                                    setToastMessage(
                                        t?.actions?.toastCopyFail
                                            ? t.actions.toastCopyFail(
                                                  err.message,
                                              )
        ...
        img.onerror = () => {
            ...
            if (setToastMessage) {
                setToastMessage(
                    t?.actions?.toastConvertFail ?? "Failed to convert image",
                );
                setToastType("error");
            }
        };
