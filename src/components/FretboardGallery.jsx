...
import {
    exportFretboardState,
    importFretboardState,
    copyToClipboard,
    readFromClipboard,
} from "../utils/fretboardShare";
...
export function FretboardGallery({
    ...
    const processImport = async (shareString, isSvg = false) => {
        try {
            ...
            onImport({
                success: false,
                message: error.message || "导入失败：未知错误",
            });
        }
    };
    ...
    const handleDialogImport = async () => {
        if (importMode === "string") {
            ...
            if (
                jsonFileInputRef.current &&
                jsonFileInputRef.current.files.length > 0
            ) {
    ...
    const handleExportDirectory = () => {
        try {
            const dirStates = historyStates.filter(
                (state) => state.directoryId === contextMenuDirectory.id,
            );
            ...
            if (onImport) {
                onImport({
                    success: true,
                    message: `已导出 ${dirStates.length} 个状态`,
                });
            }
        } catch (error) {
    ...
    const handleDirectoryDelete = (e, dir) => {
        ...
        const stateCount = historyStates.filter(
            (s) => s.directoryId === dir.id,
        ).length;
        const confirmMsg =
            stateCount > 0
                    ...
                    <div className="tabs-container">
                        {directories.map((dir) => (
                            <div
                                ...
                                onDoubleClick={(e) =>
                                    handleDirectoryDoubleClick(e, dir)
                                }
                                onContextMenu={(e) =>
                                    handleDirectoryContextMenu(e, dir)
                                }
                                onMouseEnter={() =>
                                    setHoveredDirectoryId(dir.id)
                                }
                            ...
                            >
                                {editingDirectoryId === dir.id ? (
                                    <input
                                        ...
                                        onKeyDown={(e) =>
                                            handleDirectoryNameKeyDown(e, dir)
                                        }
                                        onBlur={() =>
                                            handleDirectoryNameBlur(dir)
                                        }
                                        onClick={(e) => e.stopPropagation()}
                                        autoFocus
                                    />
                                ) : (
                                    <span className="directory-tab-name">
                                        {dir.name}
                                    </span>
                                )}
                                {!dir.isDefault &&
                                    hoveredDirectoryId === dir.id &&
                                    !editingDirectoryId && (
                                        <button
                                            className="directory-tab-close"
                                            onClick={(e) =>
                                                handleDirectoryDelete(e, dir)
                                            }
                                            title="删除目录"
                                        >
                                            ×
                    ...
                    <div className="gallery-grid">
                        {filteredStates.map((stateSnapshot) => (
                            <div
                                ...
                                onClick={(e) =>
                                    handleThumbnailClick(stateSnapshot, e)
                                }
